...
app.post('/api/payments/create',async(req,res)=>{
 ...
 try{
  ...
  for(const i of c.items)run('INSERT INTO order_items(order_id,product_id,variant_id,product_name,size,color,unit_price,quantity) VALUES(?,?,?,?,?,?,?,?)',orderId,i.product_id,i.variant_id,i.name,i.size,i.color,i.unit_price,i.quantity);
...
app.post('/api/payments/verify',(req,res)=>{
 ...
 if(!o||o.payment_reference!==razorpay_order_id||!razorpay_order_id||!razorpay_payment_id||!razorpay_signature)return res.status(400).json({error:'Invalid payment data'});
 const expected=crypto.createHmac('sha256',process.env.RAZORPAY_KEY_SECRET).update(razorpay_order_id+'|'+razorpay_payment_id).digest('hex');
 if(expected.length!==razorpay_signature.length||!crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(razorpay_signature))) {run('UPDATE orders SET payment_status=\'Failed\',updated_at=CURRENT_TIMESTAMP WHERE id=?',o.id);return res.status(400).json({error:'Payment signature verification failed'})}
 const tx=db.transaction(()=>{
   const fresh=q('SELECT * FROM orders WHERE id=?',o.id);
   if(fresh.payment_status==='Paid')return;
   for(const i of all('SELECT * FROM order_items WHERE order_id=?',o.id)){const v=q('SELECT stock FROM variants WHERE id=?',i.variant_id);if(!v||v.stock<i.quantity)throw new Error('Stock changed during checkout')}
